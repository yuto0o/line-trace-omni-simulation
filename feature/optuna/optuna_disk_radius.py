...
def run_headless_simulation(
    course_type, disk_radius, kp_trans, kd_heading, blend_both, blend_front, blend_back
):
    ...
    # DISK_RADIUS の代わりに引数の disk_radius を使用
...
# ==========================================
def objective(trial):
    # ① Optunaに探索させるパラメータの範囲を定義（disk_radiusを追加）
    disk_radius = trial.suggest_float(
        "disk_radius", 20.0, 80.0
    )  # 半径20〜80の範囲で探索
    ...
    for c_type in test_courses:
        penalty = run_headless_simulation(
            course_type=c_type,
            disk_radius=disk_radius,  # 追加したパラメータを渡す
            kp_trans=kp_trans,
